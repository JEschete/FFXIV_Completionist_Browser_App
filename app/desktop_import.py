...
def _norm_lookup_key(value: str) -> str:
    return re.sub(r"[^a-z0-9]+", "", lodestone_import._norm_label(value))
...
def _partial_match_hits_generic(
    ...
    *,
    ...
    for alias in aliases:
        norm = lodestone_import._norm_label(alias)
        if len(norm) < 8:
            continue
...
def import_desktop_completion(
    ...
    *,
...
) -> lodestone_import.ImportSummary:
    ...
    if clear_existing:
        ...
        lodestone_import.reset_character_progress(conn, character, run_id)
    ...
    for row in rows:
        ...
        for idx_label in _index_labels_for_global(
            ...
            norm = lodestone_import._norm_label(idx_label)
            if norm:
                global_norm_idx.setdefault(norm, []).append(entry)

        for bucket in lodestone_import._sheet_buckets(sheet_name):
            for idx_label in lodestone_import._index_labels_for_bucket(
                bucket=bucket,
                node_label=label,
                row_json_obj=row_json_obj,
            ):
    ...
    for candidate in candidates:
        ...
        if not hits:
            ...
            for source_label in labels:
                for base_alias in lodestone_import._candidate_aliases(bucket, source_label):
            ...
            if not hits:
                hits = lodestone_import._partial_match_hits(
                    bucket=bucket,
                    aliases=aliases,
        ...
        if not hits:
            ...
            reason, reason_extra = lodestone_import._unmatched_reason(
                bucket=bucket,
    ...
    return lodestone_import.ImportSummary(
